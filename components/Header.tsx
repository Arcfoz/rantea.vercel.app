...
import { FiBookOpen, FiHome, FiInfo, FiMenu, FiPocket } from "react-icons/fi";
import { usePathname } from "next/navigation";

function RanteaLogo() {
  return (
    <svg width="120" viewBox="0 0 250 90" xmlns="http://www.w3.org/2000/svg" className="fill-current text-[#092C2E] dark:text-white">
...
export default function Header() {
  ...
  return (
    <section className="sticky top-0 z-50 w-full  border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/95">
      <div className="m-auto flex size-full max-w-screen-2xl justify-between items-center px-3 py-6 lg:px-12 2xl:px-24">
              ...
              <div className="mt-16 mx-8 flex flex-col gap-3">
                <Link href={"/"} onClick={() => setOpen(false)}>
                  <div className="flex items-center gap-4">
                    <FiHome size={16} />
                    <h3 className="font-medium text-lg">Beranda</h3>
                  </div>
                </Link>
                <Separator />
                <Link href={"/partner"} onClick={() => setOpen(false)}>
                  <div className="flex items-center gap-4">
                    <FiPocket size={16} />
                    <h3 className="font-medium text-lg">Mitra Kami</h3>
                  </div>
                </Link>
                <Separator />
                ...
                <Link href={"/blog"} onClick={() => setOpen(false)}>
                  <div className="flex items-center gap-4">
                    <FiBookOpen size={16} />
                    <h3 className="font-medium text-lg">Artikel</h3>
                  </div>
                </Link>
              </div>
              <div className="absolute bottom-0 left-0 right-0 flex justify-center mb-8 gap-4">
                <ModeToggle />
                <Button className="hidden min-[375px]:inline-flex w-40 gap-2">
                  <MdOutlineFileDownload />
                  Unduh aplikasi
                </Button>
              </div>
